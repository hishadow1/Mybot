...
def create_embed(title, description="", color=0x1a1a1a, fields=None):
    ...
    embed.set_footer(
        text=f"StrengthCloud | VPS Manager • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        icon_url=""
    )
    return embed
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Activity(
        type=discord.ActivityType.watching, name="StrengthCloud | VPS Manager"))
...
async def show_plans(ctx):
    ...
    embed = create_embed("💎 VPS Plans - StrengthCloud", "Choose your perfect VPS plan:", 0x1a1a1a)
    plans_info = [
        ("🥉 Starter", "**RAM:** 4GB\n**CPU:** 1 Core\n**Storage:** 10GB\n**Intel:** 42 credits\n**AMD:** 83 credits"),
        ("🥈 Basic", "**RAM:** 8GB\n**CPU:** 1 Core\n**Storage:** 10GB\n**Intel:** 96 credits\n**AMD:** 164 credits"),
        ("🥇 Standard", "**RAM:** 12GB\n**CPU:** 2 Cores\n**Storage:** 10GB\n**Intel:** 192 credits\n**AMD:** 320 credits"),
...
async def bot_status(ctx):
    ...
    total_vps = sum(len(v) for v in vps_data.values())
    running_vps = sum(1 for vl in vps_data.values() for v in vl if v.get("status") == "running")
    embed = create_embed("🤖 Bot Status", "StrengthCloud VPS Manager", 0x00ff88)
...
async def announce(ctx, *, message: str):
    ...
    announce_embed.add_field(name="From", value=f"**StrengthCloud Team** ({ctx.author.mention})", inline=False)
...
async def maintenance_toggle(ctx, mode: str):
    ...
    if mode.lower() == "on":
        ...
        await bot.change_presence(
            ...
            activity=discord.Activity(type=discord.ActivityType.watching, name="StrengthCloud | VPS Manager")
        )
...
async def on_message(message):
    ...
    if isinstance(message.channel, discord.DMChannel):
        if message.content.startswith(bot.command_prefix):
            ...
            block_embed.set_footer(text="StrengthCloud | Server-only bot")
